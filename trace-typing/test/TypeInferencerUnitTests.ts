...
import * as SJS from "../src/SJS";
import {ObjectClassification} from '../src/SJSObjectClassification';
import * as State from "../src/trace-replaying/State";
import * as AST from "../src/TraceLanguageAST";
...
class Shp implements Shape {
    ...
    getPrototypeObject():Value {
        return <Primitive>{valueKind: AST.ValueKinds.Primitive, primitiveKind: AST.PrimitiveKind.Null};
    }
...
class ObjT implements ObjectType {
    ...
    public objectClassification = new TypeImpls.ObjectClassificationTypeImpl(new Set<ObjectClassification>());
...
function testAscribeObjectType(concrete:Instance, type:ObjT) {
    ...
    var actual = TypeImpls.toPrettyString(ascribedType, undefined, true);
    var expected = TypeImpls.toPrettyString(_(type), undefined, true);
