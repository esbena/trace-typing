...
function TraceBuildingAnalysis(tmpManager, astQueries, contextUtil, coercionUtil, traceBuilder, traceCollectionController, nativeSynthesisManager) {
    ...
    this.invokeFun = function (iid, f, base, args, result, isConstructor, isMethod) {
        var returnTmp = tmpManager.getResultTmp();
        if(isConstructor){
        ...
        if (f === Function.call) {
            ...
            if (contextUtil.isExternalFunction(f)) {
                ...
                traceBuilder.makeMoveStatement(returnTmp, nativeSynthesisManager.fromNative(result), iid);
                if (nativeSynthesisManager.models.requiresModel(f)) {
                    nativeSynthesisManager.models.endOperation(f, tmps.baseTmp, base, returnTmp, result, tmps.argsTmps, args, iid);
                }
            } else {
                traceBuilder.makeMoveStatement(returnTmp, tmpManager.popValueTmp(result), iid);
            }
            tmpManager.pushValueTmp(returnTmp, result, true);
        }
        var internalReturnTmp = tmpManager.getIntermediaryTmp("internal-return");
        traceBuilder.makeMoveStatement(internalReturnTmp, returnTmp, iid);
        traceBuilder.infoBuilder.makeFunctionReturn(internalReturnTmp, iid);
        traceBuilder.infoBuilder.makeFunctionResult(returnTmp, iid);
    ...
    this.literal = function (iid, val, hasGetterSetter) {
        ...
        if (isPrimitive(val)) {
            ...
            if (val instanceof RegExp || val instanceof Function) {
                ...
                var hasNumericInitializers = false;
                var hasNonNumericInitializers = false;
                for (var p in val) {
                    if (Object.prototype.hasOwnProperty.call(val, p)) {
                        hasNonNumericInitializers |= parseInt(p) + "" !== p;
                        hasNumericInitializers |= parseInt(p) + "" === p;
                        propertyNameOrder.push(p);
                    }
                }
                if(hasNumericInitializers && hasNonNumericInitializers) {
                    // order used in for-in is not the evaluation order!!
                    throw new Error("Object literals with numeric and non-numeric initializers are not supported");
                }
    ...
    this.functionExit = function (iid, returnVal, exceptionVal) {
        ...
        if (exceptionVal === undefined) {
            ...
            if (contextUtil.isCalledFromExternal()) {
                ...
                var internalReturnTmp = tmpManager.getIntermediaryTmp("internal-return");
                traceBuilder.makeMoveStatement(internalReturnTmp, returnValueTmp, iid);
                traceBuilder.infoBuilder.makeFunctionReturn(internalReturnTmp, iid);
                traceBuilder.infoBuilder.makeFunctionResult(returnValueTmp, iid);
            } else {
