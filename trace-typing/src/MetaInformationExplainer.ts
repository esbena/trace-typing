...
import * as SourceLocationImpl from "./SourceLocationImpl";
import * as AST from "./TraceLanguageAST";

import * as fs from "fs";
import * as path from "path";
import * as ncp from 'graceful-ncp';
...
class BrowserTableDisplayer {
    ...
    display(callback:Function) {
        ...
        var dir = temp.path();
        ncp.ncp(path.dirname(BrowserTableDisplayer.GUI_TABLE_LOCATION), dir, function () {
...
class BrowserStackedGroupedBarChartDisplayer {
    ...
    display(callback:Function) {
        ...
        var dir = temp.path();
        ncp.ncp(path.dirname(BrowserStackedGroupedBarChartDisplayer.GUI_CHARTS_LOCATION), dir, function () {
            var initFile = dir + '/charts-data.json';
            var chartsString = JSON.stringify(charts);
            fs.writeFile(initFile, chartsString, function (err) {
                if (err) {
                    callback(err);
                }
...
class BrowserSourceDisplayer {
    ...
    display(callback:Function) {
        ...
        var dir = temp.path();
        ncp.ncp(path.dirname(BrowserSourceDisplayer.GUI_LOCATION), dir, function (e:any) {
            if(e){
                callback(e);
                return;
            }
...
export default MetaInformationExplainerImpl;
