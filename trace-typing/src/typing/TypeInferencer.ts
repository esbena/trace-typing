...
import * as Impls from '../trace-replaying/HeapHistoryImpls';
...
/**
 * Removes the default (almost-empty) prototype if other values are present.
 */
function ignoreOverriddenDefaultPrototype(potentialPrototypes:Value[]):Value[] {
    var noDefaultPrototypes = potentialPrototypes.filter(v => {
        if (v.valueKind === AST.ValueKinds.Object) {
            var instance = <Instance>v;
            var prototypeProperties = new Set<string>();
            instance.shapes.forEach(s => s.getPropertyNames().forEach(n => prototypeProperties.add(n)));
            if (prototypeProperties.size === 0 || (prototypeProperties.size === 1 && prototypeProperties.has('constructor'))) {
                return false;
            }
        }
        return true;
    });
    if (noDefaultPrototypes.length > 0) {
        potentialPrototypes = noDefaultPrototypes;
    }
...
class TypeAscriberImpl implements TypeAscriber {
            ...
            case AST.ValueKinds.Object:
                ...
                if (this.onStack.has(instance)) {
                    ...
                    if (!this.recursiveTypeIdentifiers.has(instance)) {
                        ...
                        if(id > 100000){
                            console.warn("Aborting recursive type ascription at id: %d", id);
                            return TypeImpls.convenienceConstants.ObjectTopDueToRecursionTop;
                        }
    ...
    private ascribeObjectType(instance:Instance, path:string[]):TupleType {
        var useSJSAscription = this.useSJSAscription;
        ...
        function makeFlatProperties(instance:Instance, that:TypeAscriberImpl):PropertyTypes {
                    ...
                    shapeGroup.forEach(shape => {
                        shape.getPropertyNames().forEach(n => {
                            if (!defPresentProperties.has(n)) {
                                ...
                                if(propertyValue.valueKind === AST.ValueKinds.Primitive){
                                    var primitive: Primitive = <Primitive>propertyValue;
                                    if(primitive.primitiveKind === AST.PrimitiveKind.Null || primitive.primitiveKind === AST.PrimitiveKind.Undefined){
                                        return; // skip uninformative values
                                    }
                                }
            ...
            for (var p in properties) {
                ...
                if (property !== undefined) {
                    ...
                    if (p === 'prototype' && !useSJSAscription) {
                        property = ignoreOverriddenDefaultPrototype(property);
                    }
...
export class TypeInferencerImpl implements TypeInferencer {
    ...
    constructor(private lattice:CompleteLattice<TupleType>, private initialFunctionTypeMaker:InitialFunctionTypeMaker, useSJSAscription?:boolean) {
    ...
    inferFunctionType(signatures:DynamicFunctionSignature[], path:string[]):FunctionType {
        ...
        return this.initialFunctionTypeMaker(types);
    }
