...
function run(target, exportFileOrFunction, debug) {
    ...
    var caseDir = (target.dir ? "/" + path.basename(target.dir) + "/" : "/");
    var fullCaseDir = instrumentationDir + caseDir;
    var main = fullCaseDir + path.basename(target.main);

    if(target.dir && fs.existsSync(fullCaseDir)){
        rimraf.sync(fullCaseDir);
    }
    if(!target.dir && fs.existsSync(main)){
        rimraf.sync(main);
    }
    ...
    return jalangi.instrumentDir({
        ...
        function (options) {
            ...
            var main = fullCaseDir + path.basename(target.main);
            return jalangi.analyze(main, resolvedAnalyses, {
                debug: !!debug,
