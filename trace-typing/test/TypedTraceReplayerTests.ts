...
var precisionConfigs = {
    ...
    fullCFA: {
        ...
        callstackSensitiveVariables: {
            height: -1,
            sensitivity: TypedTraceReplayer.CallStackSensitivity.CallSite
        }
    },
    none: {
        flowInsensitiveVariables: true,
...
describe("TypedTraceReplayer", function () {
    ...
    describe("Regression for bugs", function () {
        ...
        it("Should handle weird type equality bug", function (done) {
            var config = inferenceConfigs.fullIntersection;
            var flowConfig = precisionConfigs.fullCFA;
            testFile("fixtures/bad-call.js", config, flowConfig, done);
        });
    });
    ...
    describe("Prototypes", function () {
        ...
        it('Should handle maybe shadowed prototype usage', function (done) {
            test('Object.prototype.p = "foo"; var o = {}; o.p = 42; RESULT = o.p;', function (type:TupleType) {
                assert.deepEqual(type, NumberString);
            }, config, precisionConfig, done);
        });
    });
    ...
    describe("Call stack contexts", function () {
        var config = inferenceConfigs.fullIntersection;
        var precisionConfig = precisionConfigs.fullCFA;
        ...
        it('Should merge parameters on same call stack', function (done) {
            test('function f(x){if(x === "foo"){ RESULT = x; } else { RESULT = x; };} var args = ["foo", 42]; for (var i = 0; i < args.length; i++){ f(args[i]); };', function (type:TupleType) {
                assert.deepEqual(type, Number /* recovered due to no matching signature */);
            }, config, precisionConfig, done);
        });
    });
    ...
    describe("Return types", function(){
        it('should return top for top-return-ascribed function', function (done) {
            var precisionConfig = precisionConfigs.full;
            var config = () => TypeLattices.makeSimpleSubtyping(TypeLattices.FunctionTypeLatticeKinds.FunctionPointwiseLub);
            test('function f(b){if(b){return true;}else{return "foo";}} f(true); f(false); RESULT = f(true)', function (type:TupleType) {
                assert.deepEqual(type, top);
            }, config, precisionConfig, done);
        });
        it('should use recovery for 0 function types', function (done) {
            var precisionConfig = precisionConfigs.flowInsensitive;
            var config = TypeLattices.makeSimpleSubtypingWithUnion;
            test('function f(v){return v;} f("foo"); f(true); var x = 42; x = "foo"; RESULT = f(x)', function (type:TupleType) {
                assert.deepEqual(type, OString /* recovers after failing call */);
            }, config, precisionConfig, done);
        });
        it('should return type from 1/3 function types', function (done) {
            var precisionConfig = precisionConfigs.flowInsensitive;
            var config = TypeLattices.makeSimpleSubtypingWithUnion;
            test('function f(b){if(b === "foo"){return "foo";}if(b){return 42;}else{return true;}} f("foo"); f(true); f(false); RESULT = f("foo")', function (type:TupleType) {
                assert.deepEqual(type, OString);
            }, config, precisionConfig, done);
        });
        it('should return type from 2/3 function types', function (done) {
            var precisionConfig = precisionConfigs.flowInsensitive;
            var config = TypeLattices.makeSimpleSubtypingWithUnion;
            test('function f(b){if(b === "foo"){return true;}if(b){return 42;}else{return "foo";}} f("foo"); f(true); f(false); RESULT = f(true)', function (type:TupleType) {
                assert.deepEqual(type, ONumberString);
            }, config, precisionConfig, done);
        });
        it('should return instantiated type parameter', function (done) {
            var precisionConfig = precisionConfigs.full;
            var config = () => TypeLattices.makeSimpleSubtyping(TypeLattices.FunctionTypeLatticeKinds.FunctionGenericTypeParameterOrLub);
            test('function f(v){return v;} f("foo"); f(true); RESULT = f(42)', function (type:TupleType) {
                assert.deepEqual(type, Number);
            }, config, precisionConfig, done);
        });
    });
